...
- (void)viewWillTransitionToSize:(CGSize)size withTransitionCoordinator:(id<UIViewControllerTransitionCoordinator>)coordinator {
    ...
    self->_pageIndexBeforeRotation = self->_currentPageIndex;
    self->_rotating = YES;
...
- (void)_playVideo:(NSURL *)videoURL atPhotoIndex:(NSUInteger)index {
    ...
    _currentVideoPlayerViewController.modalPresentationStyle = UIModalPresentationFormSheet;
    ...
    // Show
    [self presentViewController:_currentVideoPlayerViewController animated:NO completion:^{
        [self->_currentVideoPlayerViewController.player play];
        // workaround, as DONE button event
        [self->_currentVideoPlayerViewController addObserver:self
                                                  forKeyPath:@"view.frame"
                                                     options:NSKeyValueObservingOptionNew | NSKeyValueObservingOptionOld
                                                     context:(__bridge void * _Nullable)(self->_currentVideoPlayerViewController)];
    }];
}
...
- (void)videoFinishedCallback:(NSNotification*)notification {
    ...
    // Clear up after dismissed
    [_currentVideoPlayerViewController dismissViewControllerAnimated:NO completion:^{
        [self clearCurrentVideo];
    }];
}
...
- (void)clearCurrentVideo {
    // Remove KVO
    [_currentVideoPlayerViewController removeObserver:self
                                           forKeyPath:@"view.frame"
                                              context:(__bridge void * _Nullable)(self->_currentVideoPlayerViewController)];
...
- (void)actionButtonPressed:(id)sender {
    ...
    if ([self numberOfPhotos] > 0 && [photo underlyingImage]) {
        ...
        if ([self.delegate respondsToSelector:@selector(photoBrowser:actionButtonPressedForPhotoAtIndex:)]) {
            ...
            BOOL hasVideoURL = ([photo respondsToSelector:@selector(videoURL)] && [photo videoURL]);
            NSMutableArray *items = [NSMutableArray arrayWithObject:(isVideo && hasVideoURL) ? [photo videoURL] : [photo underlyingImage]];
            if (photo.caption) {
...
{
    if (context == (__bridge void * _Nullable)(_currentVideoPlayerViewController)) {
